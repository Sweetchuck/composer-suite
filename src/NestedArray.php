...
/**
 * Borrowed from drupal/core-utility.
 *
 * @link https://github.com/drupal/core-utility/blob/8.8.x/NestedArray.php
 */
class NestedArray
{
     ...
     * being retrieved may vary (that is, the number of parent keys is
     * variable). It is primarily used for form structures and renderable
     * arrays.
     *
     * Without this helper function the only way to get a nested array value
     * with variable depth in one line would be using eval(), which should be
     * avoided:
     *
     * @code
     ...
     * @endcode
     *
     * A return value of NULL is ambiguous, and can mean either that the
     *     requested key does not exist, or that the actual value is NULL. If
     *     it is required to know whether the nested array key actually exists,
     *     pass a third argument that is altered by reference:
     * @code
     * $key_exists = NULL;
     ...
     *
     * @return mixed
     *   The requested nested value. Possibly NULL if the value is NULL or not
     *     all nested parent keys exist. $key_exists is altered by reference
     *     and is a Boolean that indicates whether all nested parent keys exist
     *     (TRUE) or not
     ...
     *
     * This helper function should be used when the depth of the array element
     * you are changing may vary (that is, the number of parent keys is
     * variable). It is primarily used for form structures and renderable
     * arrays.
     *
     * Example:
     *
     * @code
     * // Assume you have a 'signature' element somewhere in a form. It might
     *     be:
     ...
     * @endcode
     *
     * To deal with the situation, the code needs to figure out the route to
     *     the
     ...
     * in the second case.
     *
     * Without this helper function the only way to set the signature element
     *     in
     ...
     *   requires the deletion of an already existing non-array parent value.
     *     If
     *   FALSE, PHP throws an error if trying to add into a value that is not
     *     an
     ...
     * in the second case.
     *
     * Without this helper function the only way to unset the signature element
     *     in one line would be using eval(), which should be avoided:
     * @code
     * // Do not do this! Avoid eval().
     ...
     *
     * This helper function should be used when the depth of the array element
     * to be checked may vary (that is, the number of parent keys is variable).
     * See NestedArray::setValue() for details. It is primarily used for form
     ...
     * $key_exists = array_key_exists('signature',
     ...
     *
     * @code
     * $link_options_1 = array('fragment' => 'x', 'attributes' => array('title'
     *     => t('X'), 'class' => array('a', 'b')));
     * $link_options_2 = array('fragment' => 'y', 'attributes' => array('title'
     *     => t('Y'), 'class' => array('c', 'd')));
     *
     ...
     * input arrays are passed as a single array parameter rather than a
